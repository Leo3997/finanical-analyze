...
export const LiveLogs = ({ logs }: { logs: string[] }) => {
  return (
    <Card className="terminal-panel overflow-hidden flex flex-col h-[280px] sm:h-[300px] rounded-xl" id="live-logs-panel">
      {/* Accent line */}
      ...
      <CardContent className="flex-1 p-0 overflow-hidden font-data text-[11px] sm:text-[12px]">
        <ScrollArea className="h-full px-3 sm:px-4 py-2">
          <div className="space-y-0.5">
            {logs.length > 0 ? logs.map((log, i) => {
              ...
              return (
                <div key={i} className="flex gap-1.5 sm:gap-2.5 py-0.5 animate-in fade-in slide-in-from-left-2 duration-300 hover:bg-white/[0.015] rounded px-1 -mx-1 transition-colors">
                  <span className="text-[#2a2a35] shrink-0 select-none w-6 sm:w-8 text-right">{i.toString().padStart(3, '0')}</span>
              ...
              <div className="flex items-center justify-center h-40 sm:h-48 text-[#2a2a35] px-4">
                <span className="font-data text-[10px] sm:text-xs tracking-wider text-center">
