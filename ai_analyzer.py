...
2. **报告结构**：
    ...
    - **【核心品种影响】**：重点分析新闻对玉米和鸡蛋的影响。
        - 格式：**[玉米/鸡蛋]** - 影响：[看多/看空/震荡] - 逻辑：[简述原因]。
    - **【其他品种影响矩阵】**：
...
"""
    ...
    def generate_commodity_report(self, quotes_data, news_data, keywords, spot_prices=None):
        ...
        if spot_prices:
            ...
            for item in spot_prices:
                spot_text += f"- **{item['name']}**: {item['price']} 元/吨 (日期: {item['date']}, 来源: {item['source']})\n"
...
1. **使用 Markdown 格式**。
2. **报告结构**：
    - **【当前趋势总结】**：提炼并总结当前农产品期货市场的核心运行趋势。
    - **【今日头条/核心动向】**：提取最具影响力的1-2条国内外消息。
    - **【核心品种深度分析】**：
        - **玉米**：价格走势、供需格局、政策影响、国际市场联动（CBOT玉米、USDA报告等）、期现价差
        - **鸡蛋**：价格走势、存栏量变化、季节性因素、饲料成本影响、期现价差
    - **【农产品板块分析】**：
        - **蛋白粕**：豆粕、菜粕等品种分析
        - **油脂**：豆油、棕榈油、菜籽油等品种分析
        - **谷物**：淀粉、大豆等品种分析
        ...
        - **畜牧**：生猪等品种分析
    - **【期现价差分析】**：基于现货价格数据，分析主要品种的基差（期货-现货）情况。
